...
def get_last_entry_date(source_name):
    try:
        metadb = connect_db()
        date = "0001-01-01T05:15Z"
        if metadb.count_documents({}) > 0:
            temp = list(
                metadb.find(
                    {"CVE-id": {"$regex": source_name, "$options": "i"}}, {"_id": 0, "Date": 1}).sort("Date", -1).limit(1))
            if not temp == []:
                date = temp[0]["Date"]
        return date
    except Exception as error:
        print("Error while fetching last published date entry: ", error)
        log_progress("Error while fetching last published date entry: " + str(error), "ERROR")
        raise error
